...
Модель Б. Боэма — модель оценки качества ПО, которая позволяет более точно определять качество его основных характеристик, заданных набором показателей и метрик (т.е. учитывает объем кода, и сложность модуля определяется числом строк). 
